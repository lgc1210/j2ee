...
const ProfileChangePassword = () => {
	...
	const handleSubmit = async (event) => {
		...
		try {
			...
			console.log("Errors occur while changing password", error);
			if (error?.response?.status === 400) {
				showToast(error.response?.data, "error");
			} else {
				showToast("An unexpected error occured. Please try again later.", "error");
			}
		}
	};
			...
			<div className=''>
				...
				<div className='flex flex-col gap-4'>
					...
					<div className='p-4 rounded-md border border-black/10'>
						...
						<form className='mt-4 flex flex-col gap-4 min-w-fit w-1/3' onSubmit={handleSubmit}>
							<div>
								<FormControl
									type='password'
									placeHolder='Enter your current password'
									wrapInputStyle={`rounded !border-black/20 focus-within:!border-black/60 transition-all ${
										errors?.currentPassword ? "!border-red-400 focus-within:!border-red-400" : ""
									}`}
									...
									onChange={(event) => handleFieldsChange("currentPassword", event.target.value)}
									onType={() => handleFieldsType("currentPassword")}
									onBlur={() => {
										if (isEmpty(fields.currentPassword))
											handleFieldsBlur("currentPassword", "Current password is required");
									}}
							...
							<div>
								<FormControl
									...
									wrapInputStyle={`rounded !border-black/20 focus-within:!border-black/60 transition-all ${
										errors?.newPassword ? "!border-red-400 focus-within:!border-red-400" : ""
									}`}
									...
									id='newPassword'
									labelStyle='mb-1 font-serif'
									onChange={(event) => handleFieldsChange("newPassword", event.target.value)}
									onType={() => handleFieldsType("newPassword")}
									onBlur={() => {
										if (isEmpty(fields.newPassword))
											handleFieldsBlur("newPassword", "New password is required");
									}}
							...
							<div>
								<FormControl
									...
									wrapInputStyle={`rounded !border-black/20 focus-within:!border-black/60 transition-all ${
										errors?.confirmPassword ? "!border-red-400 focus-within:!border-red-400" : ""
									}`}
									inputStyle='rounded border-black/60 placeholder:text-lg text-black placeholder:font-serif'
									hasLabel
									...
									id='confirmPassword'
									labelStyle='mb-1 font-serif'
									onChange={(event) => handleFieldsChange("confirmPassword", event.target.value)}
									onType={() => handleFieldsType("confirmPassword")}
									onBlur={() => {
										if (!isConfirmPassword(fields.newPassword, fields.confirmPassword))
											handleFieldsBlur("confirmPassword", "Confirm password does not match");
									}}
