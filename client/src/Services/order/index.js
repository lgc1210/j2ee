...
class OrderService {
	createOrder = async (payload) => {
		return await apiInstance.post(`${baseURL}/orders`, payload);
	};

	getOrderStats = async (timeFilter, specificFilter) => {
		return await apiInstance.get(
			`${baseURL}/order/filter_orders?filter=${timeFilter}${
	...
	getOrdersByStore = async (storeId) => {
		return await apiInstance.get(`${baseURL}/orders/store/${storeId}`, {
			headers: {
				"Content-Type": "application/json",
			},
		});
